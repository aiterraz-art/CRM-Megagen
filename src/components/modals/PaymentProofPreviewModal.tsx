...
const isPdfFile = (fileType: string | null, fileName?: string | null) => {
...
const isImageFile = (fileType: string | null, fileName?: string | null) => {
    ...
    return normalizedType.startsWith('image/')
        || ['.jpg', '.jpeg', '.png', '.webp'].some((ext) => normalizedName.endsWith(ext));
};
...
const PaymentProofPreviewModal = ({
    ...
    const showPdf = Boolean(blobUrl && isPdfFile(fileType, fileName));
    const showImage = Boolean(blobUrl && isImageFile(fileType, fileName));
    const showUnsupported = Boolean(blobUrl) && !showPdf && !showImage;
                ...
                <div className="flex-1 min-h-0 bg-gray-50 p-4 md:p-6 overflow-auto">
                    {loading ? (
                        <div className="h-full min-h-[320px] flex flex-col items-center justify-center text-gray-500">
                            ...
                            <p className="text-sm font-bold">Descargando comprobante...</p>
                        </div>
                    ) : error ? (
                        <div className="h-full min-h-[320px] flex items-center justify-center">
                            <div className="max-w-md w-full rounded-[1.5rem] border border-red-100 bg-white p-6 text-center shadow-sm">
                                ...
                                <h4 className="text-base font-black text-gray-900">No se pudo cargar el comprobante</h4>
                        ...
                        <div className="h-full min-h-[65vh] rounded-[1.5rem] border border-gray-200 bg-white overflow-hidden shadow-sm">
                            <iframe
                                src={blobUrl}
                                title={`Comprobante pedido ${orderFolio ?? ''}`}
                                className="w-full h-[65vh] md:h-[72vh]"
                            />
                        </div>
                    ) : showImage && blobUrl ? (
                        <div className="min-h-[320px] rounded-[1.5rem] border border-gray-200 bg-white overflow-auto shadow-sm p-4 flex items-center justify-center">
                            <img
                                src={blobUrl}
                                alt={`Comprobante pedido ${orderFolio ?? ''}`}
                                className="max-h-[72vh] w-auto max-w-full object-contain rounded-xl"
                            />
                        </div>
                    ) : showUnsupported ? (
                        <div className="h-full min-h-[320px] flex items-center justify-center">
                            <div className="max-w-md w-full rounded-[1.5rem] border border-amber-100 bg-white p-6 text-center shadow-sm">
                                <AlertCircle size={28} className="mx-auto text-amber-500 mb-3" />
                                <h4 className="text-base font-black text-gray-900">No se puede previsualizar este archivo</h4>
                                <p className="text-sm text-gray-600 font-medium mt-2">
                                    El comprobante fue descargado correctamente, pero este tipo de archivo no tiene vista previa dentro del CRM.
                                </p>
                            </div>
                        </div>
                    ) : (
                        <div className="h-full min-h-[320px] flex items-center justify-center">
                            <div className="max-w-md w-full rounded-[1.5rem] border border-gray-200 bg-white p-6 text-center shadow-sm">
                                ...
                                <h4 className="text-base font-black text-gray-900">Sin comprobante disponible</h4>
                                <p className="text-sm text-gray-500 font-medium mt-2">Este pedido no tiene un comprobante cargado para mostrar.</p>
                            </div>
                        </div>
                    )}
                </div>
