...
BEGIN
  ...
  FROM (
    SELECT
      ...
      lower(trim(NULLIF(
        CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>4 ELSE element.value->>'document_number' END,
        ''
      ))) AS normalized_document_number,
      regexp_replace(lower(COALESCE(NULLIF(trim(
        CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>3 ELSE element.value->>'client_rut' END
      ), ''), '')), '[^0-9k]', '', 'g') AS normalized_client_rut,
      NULLIF(lower(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>0 ELSE element.value->>'seller_email' END)), '') AS seller_email,
      NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>1 ELSE element.value->>'seller_name' END), '') AS seller_name,
      NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>2 ELSE element.value->>'client_name' END), '') AS client_name,
      NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>3 ELSE element.value->>'client_rut' END), '') AS client_rut,
      NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>4 ELSE element.value->>'document_number' END), '') AS document_number,
      COALESCE(NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>5 ELSE element.value->>'document_type' END), ''), 'invoice') AS document_type,
      NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>6 ELSE element.value->>'issue_date' END), '')::date AS issue_date,
      NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>7 ELSE element.value->>'due_date' END), '')::date AS due_date,
      GREATEST(COALESCE(NULLIF(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>8 ELSE element.value->>'amount' END, '')::numeric, 0), 0) AS amount,
      GREATEST(COALESCE(NULLIF(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>9 ELSE element.value->>'outstanding_amount' END, '')::numeric, NULLIF(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>8 ELSE element.value->>'amount' END, '')::numeric, 0), 0) AS outstanding_amount,
      CASE
        WHEN lower(COALESCE(NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>10 ELSE element.value->>'status' END), ''), 'pending')) IN ('pending', 'partial', 'paid', 'overdue', 'disputed')
          THEN lower(COALESCE(NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>10 ELSE element.value->>'status' END), ''), 'pending'))
        ELSE 'pending'
      END AS status,
      NULLIF(trim(CASE WHEN jsonb_typeof(element.value) = 'array' THEN element.value->>11 ELSE element.value->>'notes' END), '') AS notes
