...
BEGIN
    ...
    IF EXISTS (
        ...
        JOIN public.delivery_routes active_routes
         ...
         AND lower(coalesce(active_routes.status, '')) = 'in_progress'
...
BEGIN
    ...
    IF lower(coalesce(v_route.status, '')) = 'completed' THEN
        RETURN jsonb_build_object(
            'route_id', v_route.id,
            'status', 'completed'
        );
    ...
    IF lower(coalesce(v_route.status, '')) <> 'in_progress' THEN
    ...
    WHERE route_id = p_route_id
      AND lower(coalesce(status, '')) IN ('pending', 'rescheduled', 'failed');
